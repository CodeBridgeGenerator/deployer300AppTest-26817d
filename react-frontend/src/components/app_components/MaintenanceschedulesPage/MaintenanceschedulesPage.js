...
const MaintenanceschedulesPage = (props) => {
  ...
  const filename = "maintenanceschedules";
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("maintenanceschedules").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="maintenanceschedules-add-button"
            />
          </>
        </div>
      </div>
